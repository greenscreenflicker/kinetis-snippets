...
void Si8900_hw_init(uint32_t baudrate){
   ...
   PORTD->PCR[3] = PORT_PCR_MUX(3);
	...
	//Enable error interrupt, that recovers UART.
	   //framing, parity, noise, overrun interrupt enable
	UART2->C3= UART_C3_FEIE_MASK|UART_C3_PEIE_MASK|UART_C3_NEIE_MASK|UART_C3_ORIE_MASK;
...
void SI8900_autobaudrate_detection(void){
	...
	SI8900_sync_flag=0;
	DMA0->TCD[4].CSR|=DMA_CSR_INTMAJOR_MASK;
...
//resets buffer, so that data isn't treated as new data.
void SI8900_void_data(void){
	SI8900_rec_buf[3*0]=0;
	SI8900_rec_buf[3*1]=0;
	SI8900_rec_buf[3*2]=0;
}
